...
BOT1_CONFIG = {
    'name': 'Bot 1 - MSR + VTP',
    'symbols': {
        'MSR': {'sl': 8.0, 'tp': 16.0, 'holding': 28, 'pullback': 0.08, 'ma100': True},
        'VTP': {'sl': 8.0, 'tp': 16.0, 'holding': 20, 'pullback': 0.08, 'ma100': False}
    }
}
...
def scan_signals():
    tz_vn = pytz.timezone('Asia/Ho_Chi_Minh')
    print(f"\n🔍 [{datetime.now(tz_vn).strftime('%H:%M:%S %d/%m/%Y')}] Quét {BOT1_CONFIG['name']}...")
    
    for symbol, cfg in BOT1_CONFIG['symbols'].items():
        try:
            ...
            # Kiểm tra bộ lọc Pullback & MA100
